import type { Event, Theater, Reservation, Review } from '../types'

export const mockTheaters: Theater[] = [
  {
    id: '1',
    name: 'HNK Zagreb',
    address: 'Trg Republike Hrvatske 15, Zagreb',
    description: 'Hrvatsko narodno kazalište u Zagrebu, utemeljena 1860. godine, jedna je od najznačajnijih kulturnih institucija u Hrvatskoj.',
    capacity: 680,
    contact: '+385 1 4888 418',
    workingHours: 'Pon-Pet: 10:00-20:00, Sub: 10:00-14:00',
    latitude: 45.8094,
    longitude: 15.9694,
  },
  {
    id: '2',
    name: 'Kazalište Gavella',
    address: 'Frankopanska 8, Zagreb',
    description: 'Gradsko dramsko kazalište Gavella jedno je od vodećih dramskih kazališta u Hrvatskoj.',
    capacity: 350,
    contact: '+385 1 4849 222',
    workingHours: 'Pon-Pet: 10:00-19:00',
    latitude: 45.8117,
    longitude: 15.9703,
  },
  {
    id: '3',
    name: 'Kerempuh',
    address: 'Ilica 31, Zagreb',
    description: 'Satiričko kazalište Kerempuh poznato je po satiričnim i komičnim predstavama.',
    capacity: 200,
    contact: '+385 1 4833 354',
    workingHours: 'Pon-Pet: 10:00-19:00',
    latitude: 45.8131,
    longitude: 15.9715,
  },
  {
    id: '4',
    name: 'Teatar &TD',
    address: 'Savska cesta 25, Zagreb',
    description: 'Studentski centar Zagreb, prostor za suvremene i eksperimentalne izvedbe.',
    capacity: 150,
    contact: '+385 1 4593 666',
    workingHours: 'Pon-Sub: 12:00-20:00',
    latitude: 45.8056,
    longitude: 15.9666,
  },
]

export const mockEvents: Event[] = [
  {
    id: '1',
    title: 'Hamlet',
    description: 'Shakespeareova najpoznatija tragedija u novoj, modernoj interpretaciji. Princ Hamlet suočava se s izdajom, osvetom i pitanjima o smislu postojanja.',
    date: '2026-03-20',
    time: '19:30',
    theaterId: '1',
    theaterName: 'HNK Zagreb',
    pricePerTicket: 25,
    totalSeats: 680,
    availableSeats: 142,
    imageUrl: 'https://images.unsplash.com/photo-1507676184212-d03ab07a01bf?w=600&h=400&fit=crop',
    duration: '2h 30min',
  },
  {
    id: '2',
    title: 'Čekajući Godota',
    description: 'Samuel Beckett nas vodi u apsurdni svijet dvaju skitnica koji čekaju nekoga tko možda nikad neće doći. Klasik teatra apsurda.',
    date: '2026-03-22',
    time: '20:00',
    theaterId: '2',
    theaterName: 'Kazalište Gavella',
    pricePerTicket: 18,
    totalSeats: 350,
    availableSeats: 89,
    imageUrl: 'https://images.unsplash.com/photo-1503095396549-807759245b35?w=600&h=400&fit=crop',
    duration: '1h 45min',
  },
  {
    id: '3',
    title: 'Tko se boji Virginije Woolf?',
    description: 'Edward Albeeova drama o braku, iluzijama i razornoj moći istine. Dva para, jedna noć, bezbroj tajni.',
    date: '2026-03-25',
    time: '19:00',
    theaterId: '2',
    theaterName: 'Kazalište Gavella',
    pricePerTicket: 20,
    totalSeats: 350,
    availableSeats: 210,
    imageUrl: 'https://images.unsplash.com/photo-1460723237483-7a6dc9d0b212?w=600&h=400&fit=crop',
    duration: '2h 15min',
  },
  {
    id: '4',
    title: 'Revizor',
    description: 'Gogoljeva satirična komedija o korumpiranom gradiću koji posjećuje lažni državni revizor. Smiješno i bolno aktualno.',
    date: '2026-03-28',
    time: '19:30',
    theaterId: '3',
    theaterName: 'Kerempuh',
    pricePerTicket: 15,
    totalSeats: 200,
    availableSeats: 45,
    imageUrl: 'https://images.unsplash.com/photo-1514306191717-452ec28c7814?w=600&h=400&fit=crop',
    duration: '2h',
  },
  {
    id: '5',
    title: 'Medeja',
    description: 'Euripidova antička tragedija o ženi ostavljenoj od muža i njezinoj strašnoj osveti. Snaga emocija koja nadilazi vrijeme.',
    date: '2026-04-02',
    time: '20:00',
    theaterId: '1',
    theaterName: 'HNK Zagreb',
    pricePerTicket: 22,
    totalSeats: 680,
    availableSeats: 380,
    imageUrl: 'https://images.unsplash.com/photo-1580809361436-42a7ec204889?w=600&h=400&fit=crop',
    duration: '1h 50min',
  },
  {
    id: '6',
    title: 'Kiklop',
    description: 'Dramatizacija romana Ranka Marinkovića. Zagreb pred Drugi svjetski rat, intelektualci, strah i apsurd svakodnevice.',
    date: '2026-04-05',
    time: '19:00',
    theaterId: '2',
    theaterName: 'Kazalište Gavella',
    pricePerTicket: 20,
    totalSeats: 350,
    availableSeats: 175,
    imageUrl: 'https://images.unsplash.com/photo-1598387993441-a364f854c3e1?w=600&h=400&fit=crop',
    duration: '2h 20min',
  },
  {
    id: '7',
    title: 'Dundo Maroje',
    description: 'Marinova komedija Držića u punom sjaju! Rim, škrtost, ljubav i intrige — renesansna komedija koja nikad ne stari.',
    date: '2026-04-10',
    time: '19:30',
    theaterId: '1',
    theaterName: 'HNK Zagreb',
    pricePerTicket: 25,
    totalSeats: 680,
    availableSeats: 290,
    imageUrl: 'https://images.unsplash.com/photo-1547153760-18fc86324498?w=600&h=400&fit=crop',
    duration: '2h 10min',
  },
  {
    id: '8',
    title: 'Postolar i vrag',
    description: 'Brechtova drama o čovjeku koji sklopi pakt s vragom. Pitanje morala, pohlepe i ljudske prirode.',
    date: '2026-04-12',
    time: '20:00',
    theaterId: '3',
    theaterName: 'Kerempuh',
    pricePerTicket: 15,
    totalSeats: 200,
    availableSeats: 120,
    imageUrl: 'https://images.unsplash.com/photo-1568037027677-407cba5c3539?w=600&h=400&fit=crop',
    duration: '1h 40min',
  },
  {
    id: '9',
    title: 'Tri sestre',
    description: 'Čehovljeva drama o čežnji za boljim životom. Tri sestre sanjaju o Moskvi dok im život prolazi u provinciji.',
    date: '2026-04-15',
    time: '19:00',
    theaterId: '4',
    theaterName: 'Teatar &TD',
    pricePerTicket: 12,
    totalSeats: 150,
    availableSeats: 67,
    imageUrl: 'https://images.unsplash.com/photo-1524368535928-5b5e00ddc76b?w=600&h=400&fit=crop',
    duration: '2h 30min',
  },
]

export const mockReservations: Reservation[] = [
  {
    id: '1',
    userId: '1',
    eventId: '1',
    eventTitle: 'Hamlet',
    theaterName: 'HNK Zagreb',
    date: '2026-03-20',
    time: '19:30',
    numberOfTickets: 2,
    totalPrice: 50,
    status: 'confirmed',
    createdAt: '2026-03-10T14:30:00',
  },
  {
    id: '2',
    userId: '1',
    eventId: '3',
    eventTitle: 'Tko se boji Virginije Woolf?',
    theaterName: 'Kazalište Gavella',
    date: '2026-03-25',
    time: '19:00',
    numberOfTickets: 4,
    totalPrice: 80,
    status: 'confirmed',
    createdAt: '2026-03-11T09:15:00',
  },
  {
    id: '3',
    userId: '1',
    eventId: '4',
    eventTitle: 'Revizor',
    theaterName: 'Kerempuh',
    date: '2026-03-28',
    time: '19:30',
    numberOfTickets: 1,
    totalPrice: 15,
    status: 'cancelled',
    createdAt: '2026-03-08T18:45:00',
  },
  {
    id: '4',
    userId: '1',
    eventId: '5',
    eventTitle: 'Medeja',
    theaterName: 'HNK Zagreb',
    date: '2026-04-02',
    time: '20:00',
    numberOfTickets: 3,
    totalPrice: 66,
    status: 'confirmed',
    createdAt: '2026-03-09T11:00:00',
  },
  {
    id: '5',
    userId: '1',
    eventId: '7',
    eventTitle: 'Dundo Maroje',
    theaterName: 'HNK Zagreb',
    date: '2026-04-10',
    time: '19:30',
    numberOfTickets: 2,
    totalPrice: 50,
    status: 'cancelled',
    createdAt: '2026-03-07T16:20:00',
  },
  {
    id: '6',
    userId: '1',
    eventId: '9',
    eventTitle: 'Tri sestre',
    theaterName: 'Teatar &TD',
    date: '2026-04-15',
    time: '19:00',
    numberOfTickets: 2,
    totalPrice: 24,
    status: 'confirmed',
    createdAt: '2026-03-12T10:30:00',
  },
]

export const mockReviews: Review[] = [
  {
    id: '1',
    userId: '1',
    userName: 'Ana Horvat',
    userEmail: 'ana.horvat@email.hr',
    eventId: '1',
    eventTitle: 'Hamlet',
    theaterName: 'HNK Zagreb',
    rating: 5,
    comment: 'Izvrsna predstava! Glumci su bili fenomenalni, posebno u sceni s lubanjom. Topla preporuka svima koji vole klasiku.',
    createdAt: '2026-03-05T20:30:00',
  },
  {
    id: '2',
    userId: '1',
    userName: 'Ana Horvat',
    userEmail: 'ana.horvat@email.hr',
    eventId: '4',
    eventTitle: 'Revizor',
    theaterName: 'Kerempuh',
    rating: 4,
    comment: 'Odlična satira, publika se smijala od početka do kraja. Jedina zamjerka — malo predug drugi čin.',
    createdAt: '2026-03-02T21:00:00',
  },
  {
    id: '3',
    userId: '2',
    userName: 'Marko Jurić',
    userEmail: 'marko.juric@email.hr',
    eventId: '1',
    eventTitle: 'Hamlet',
    theaterName: 'HNK Zagreb',
    rating: 4,
    comment: 'Moderna interpretacija koja funkcionira. Scenografija je bila impresivna.',
    createdAt: '2026-03-06T19:45:00',
  },
  {
    id: '4',
    userId: '2',
    userName: 'Marko Jurić',
    userEmail: 'marko.juric@email.hr',
    eventId: '2',
    eventTitle: 'Čekajući Godota',
    theaterName: 'Kazalište Gavella',
    rating: 5,
    comment: 'Remek-djelo! Gavella nikad ne razočara. Beckett u punom sjaju.',
    createdAt: '2026-03-01T22:00:00',
  },
  {
    id: '5',
    userId: '3',
    userName: 'Petra Novak',
    userEmail: 'petra.novak@email.hr',
    eventId: '9',
    eventTitle: 'Tri sestre',
    theaterName: 'Teatar &TD',
    rating: 3,
    comment: 'Solidna izvedba, ali intimni prostor Teatra &TD nije idealan za Čehova. Glumice su bile sjajne.',
    createdAt: '2026-02-28T20:15:00',
  },
  {
    id: '6',
    userId: '3',
    userName: 'Petra Novak',
    userEmail: 'petra.novak@email.hr',
    eventId: '5',
    eventTitle: 'Medeja',
    theaterName: 'HNK Zagreb',
    rating: 5,
    comment: 'Potresno i snažno. Glavna glumica je bila nevjerojatna — suze u publici.',
    createdAt: '2026-03-04T21:30:00',
  },
]
